...
update `logistics_product`  lp
inner join `shop_product_sku`  sps on(lp.`shop_product_sku_id` = sps.`shop_product_sku_id`  and sps.`enabled`  = 1 and lp.`enabled`  = 1 )
inner join `sku`  s on(s.`sku_id` = sps.`sku_id`  and s.`enabled`  = 1 )
set lp.`reatil_price` = s.`price`  ;
...
//          });
    ...
    methods: {
      ...
      getSesaon(val){
        queryRuleByNotHasSesaon(val, 1).then(res => { //COPY TO NEW data
      ...
      getTablenav(val){
        ...
        queryRuleByHasSeason(1, val, 1).then(res => { //获取head tab
...
//        }
      ...
      createSeason(type){
        if (type === 'CREATE') {
          ...
          this.showShade = false;
          this.isLoading = true;
          copyseasonVendor(this.priceId, this.arrSelectSeason.join(","), this.boutiqueVendorid, 1).then(res => {
            if (res.data.status === 1) {
              this.getTablenav(this.boutiqueVendorid)
            } else {
              if (!this.imAlert.show) {

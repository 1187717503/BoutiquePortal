...
<script>
  ...
  export default {
    ...
    mounted() {
      ...
      selectActiveBrands().then(res => { //获取Brands
...
//          });
    ...
    methods: {
      ...
      getSesaon(val){
        queryRuleByNotHasSesaon(val, 3).then(res => { //COPY TO NEW data
      ...
      getTablenav(val){
        ...
        queryRuleByHasSeason(1, val, 3).then(res => { //获取head tab
      ...
      setCategory(event, val, id){//设置三级选择框
        this.categoryName = val
        this.addCategoryid = id
        this.isMultistep = false
      },
      hideMultistep(){ //关闭三级选择框
        setTimeout(() => {
          this.isMultistep = false
        }, 300)
      },
      ...
      searchBandleft(event){
        let lDom = event.target.parentNode.parentNode.parentNode.children[2].children[0].children
...
//      编辑
      ...
      delBrand(id) {
        ...
        let data = {
          "price_change_rule_id": this.priceId,
          "price_change_rule_category_brand_list": [
            id
          ]
        }
...
//        正确写法
        this.listBandright = Object.assign({}, this.listBandright, this.addBandright)
//        去重
        let ret = [];
      ...
      delBand(event){
        ...
        if (event.target.parentNode.parentNode.children[2].children[0].children[0].children[0].checked) {
          this.listBandright = [];
        } else {
          for (let i = 0; i < checkDom.length; i++) {
            if (checkDom[i].children[0].children[0].checked) {
              for (let k in this.dellistBandright) {
                for (let j in this.listBandright) {
                  if (this.dellistBandright[k].brand_id === this.listBandright[j].brand_id) {
      ...
      delBrandCateort(item){
        this.isLoading = true
        let data = {
          "price_change_rule_category_brand_id": item.price_change_rule_category_brand_id
        }
...
//        }
      ...
      createPricingRule(){
        ...
        if (this.pricingRule === 0) {
          activeVendor(this.allvendorId, this.discount === null ? 0 : this.discount, this.boutiqueVendorid).then(res => {
            if (res.data.status === 1) {
              this.getTablenav(this.boutiqueVendorid)
              this.showCopy = false;
              this.showShade = false;
        ...
        if (this.pricingRule === 1) {
          pengingVendor(this.discount === null ? 0 : this.discount, this.boutiqueVendorid).then(res => {
            if (res.data.status === 1) {
              this.getTablenav(this.boutiqueVendorid)
              this.showCopy = false;
              this.showShade = false;
      ...
      createSeason(type){
        if (type === 'CREATE') {
          ...
          this.showShade = false;
          this.isLoading = true;
          copyseasonVendor(this.priceId, this.arrSelectSeason.join(","), this.boutiqueVendorid, 3).then(res => {
            if (res.data.status === 1) {
              this.getTablenav(this.boutiqueVendorid)
              this.arrSelectSeason = [];
            } else {

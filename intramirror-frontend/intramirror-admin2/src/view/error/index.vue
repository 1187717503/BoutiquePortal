<template>
  <div>
    <div class="head-search">
      ...
      <div class="input-field">
        <select id="Boutique">
          <option v-for="item in vendorList" :value="item.vendor_id+','+item.vendor_name">{{item.vendor_name}}</option>
        </select>
...
<script>
  ...
  import {getApiErrorPage, searchApiError, selectShowArr, processing, remove} from '../../api/error'
  ...
  export default {
    data() {
      return {
        ...
        tabHead: [
          {text: 'Error ID', align: 'center', sortable: false, value: 'error_id'},
          {text: 'Boutique', value: 'vendor_name', align: 'center', sortable: false,},
          {text: 'API', value: 'name', align: 'center', sortable: false,},
          {text: 'Brand ID', value: 'brand_id', align: 'center', sortable: false,},
          {text: 'Color', value: 'color_code', align: 'center', sortable: false,},
          {text: 'Boutique ID', value: 'BoutiqueID', align: 'center', sortable: false,},
          {text: 'Error Type', value: 'error_type_name', align: 'center', sortable: false,},
          {text: 'Data Field', value: 'data_field', align: 'center', sortable: false,},
          {text: 'Boutique Data', value: 'boutique_data', align: 'center', sortable: false,},
          {text: 'Create DateTime', value: 'create_time', align: 'center', sortable: false,},
          {text: 'Process DateTime', value: 'process_time', align: 'center', sortable: false,}
        ],
        items: []
      }
    },
    ...
    mounted() {
      ...
      flatpickr('#date', {
        ...
        onClose(e, s) {
          ...
          _this.searchData.Date = star.getFullYear() + '-' + parseInt(star.getMonth() + 1) + '-' + star.getDate() + ' 00:00:00' + '~' + end.getFullYear() + '-' + parseInt(end.getMonth() + 1) + '-' + end.getDate() + ' 23:59:59';
        }
      });
      this.loadBoutique();
      this.loadList();
      ...
      let isClick = [false, false, false, false, false, false, false, false, false, false, false];

      $('.sort thead .column').eq(0).on('click', () => {
        this.delDom();
        this.sortType = 'errorId';
        if (isClick[0]) {
          this.sortNum = 0;
          $('.sort thead .column').eq(0).append("<i aria-hidden='true' class='material-icons icon'>arrow_upward</i>");
          $('.sort thead .column').eq(0).addClass('sortable active asc desc');
          isClick[0] = false;
        } else {
          this.sortNum = 1;
          $('.sort thead .column').eq(0).append("<i aria-hidden='true' class='material-icons icon'>arrow_upward</i>");
          $('.sort thead .column').eq(0).addClass('sortable active asc');
          isClick[0] = true;
        }
        this.loadList();
      });

      $('.sort thead .column').eq(1).on('click', () => {
        this.delDom();
        this.sortType = 'boutique';
        if (isClick[1]) {
          this.sortNum = 0;
          $('.sort thead .column').eq(1).append("<i aria-hidden='true' class='material-icons icon'>arrow_upward</i>");
          $('.sort thead .column').eq(1).addClass('sortable active asc desc');
          isClick[1] = false;
        } else {
          this.sortNum = 1;
          $('.sort thead .column').eq(1).append("<i aria-hidden='true' class='material-icons icon'>arrow_upward</i>");
          $('.sort thead .column').eq(1).addClass('sortable active asc');
          isClick[1] = true;
        }
        this.loadList();
      });

      $('.sort thead .column').eq(2).on('click', () => {
        this.delDom();
        this.sortType = 'api';
        if (isClick[2]) {
          this.sortNum = 0;
          $('.sort thead .column').eq(2).append("<i aria-hidden='true' class='material-icons icon'>arrow_upward</i>");
          $('.sort thead .column').eq(2).addClass('sortable active asc desc');
          isClick[2] = false;
        } else {
          this.sortNum = 1;
          $('.sort thead .column').eq(2).append("<i aria-hidden='true' class='material-icons icon'>arrow_upward</i>");
          $('.sort thead .column').eq(2).addClass('sortable active asc');
          isClick[2] = true;
        }
        this.loadList();
      });

      $('.sort thead .column').eq(3).on('click', () => {
        this.delDom();
        this.sortType = 'brandId';
        if (isClick[3]) {
          this.sortNum = 0;
          $('.sort thead .column').eq(3).append("<i aria-hidden='true' class='material-icons icon'>arrow_upward</i>");
          $('.sort thead .column').eq(3).addClass('sortable active asc desc');
          isClick[3] = false;
        } else {
          this.sortNum = 1;
          $('.sort thead .column').eq(3).append("<i aria-hidden='true' class='material-icons icon'>arrow_upward</i>");
          $('.sort thead .column').eq(3).addClass('sortable active asc');
          isClick[3] = true;
        }
        this.loadList();
      });

      $('.sort thead .column').eq(4).on('click', () => {
        this.delDom();
        this.sortType = 'color';
        if (isClick[4]) {
          this.sortNum = 0;
          $('.sort thead .column').eq(4).append("<i aria-hidden='true' class='material-icons icon'>arrow_upward</i>");
          $('.sort thead .column').eq(4).addClass('sortable active asc desc');
          isClick[4] = false;
        } else {
          this.sortNum = 1;
          $('.sort thead .column').eq(4).append("<i aria-hidden='true' class='material-icons icon'>arrow_upward</i>");
          $('.sort thead .column').eq(4).addClass('sortable active asc');
          isClick[4] = true;
        }
        this.loadList();
      });

      $('.sort thead .column').eq(5).on('click', () => {
        this.delDom();
        this.sortType = 'boutiqueId';
        if (isClick[5]) {
          this.sortNum = 0;
          $('.sort thead .column').eq(5).append("<i aria-hidden='true' class='material-icons icon'>arrow_upward</i>");
          $('.sort thead .column').eq(5).addClass('sortable active asc desc');
          isClick[5] = false;
        } else {
          this.sortNum = 1;
          $('.sort thead .column').eq(5).append("<i aria-hidden='true' class='material-icons icon'>arrow_upward</i>");
          $('.sort thead .column').eq(5).addClass('sortable active asc');
          isClick[5] = true;
        }
        this.loadList();
      });

      $('.sort thead .column').eq(6).on('click', () => {
        this.delDom();
        this.sortType = 'errorType';
        if (isClick[6]) {
          this.sortNum = 0;
          $('.sort thead .column').eq(6).append("<i aria-hidden='true' class='material-icons icon'>arrow_upward</i>");
          $('.sort thead .column').eq(6).addClass('sortable active asc desc');
          isClick[6] = false;
        } else {
          this.sortNum = 1;
          $('.sort thead .column').eq(6).append("<i aria-hidden='true' class='material-icons icon'>arrow_upward</i>");
          $('.sort thead .column').eq(6).addClass('sortable active asc');
          isClick[6] = true;
        }
        this.loadList();
      });

      $('.sort thead .column').eq(7).on('click', () => {
        this.delDom();
        this.sortType = 'dateField';
        if (isClick[7]) {
          this.sortNum = 0;
          $('.sort thead .column').eq(7).append("<i aria-hidden='true' class='material-icons icon'>arrow_upward</i>");
          $('.sort thead .column').eq(7).addClass('sortable active asc desc');
          isClick[7] = false;
        } else {
          this.sortNum = 1;
          $('.sort thead .column').eq(7).append("<i aria-hidden='true' class='material-icons icon'>arrow_upward</i>");
          $('.sort thead .column').eq(7).addClass('sortable active asc');
          isClick[7] = true;
        }
        this.loadList();
      });

      $('.sort thead .column').eq(8).on('click', () => {
        this.delDom();
        this.sortType = 'boutiqueDate';
        if (isClick[8]) {
          this.sortNum = 0;
          $('.sort thead .column').eq(8).append("<i aria-hidden='true' class='material-icons icon'>arrow_upward</i>");
          $('.sort thead .column').eq(8).addClass('sortable active asc desc');
          isClick[8] = false;
        } else {
          this.sortNum = 1;
          $('.sort thead .column').eq(8).append("<i aria-hidden='true' class='material-icons icon'>arrow_upward</i>");
          $('.sort thead .column').eq(8).addClass('sortable active asc');
          isClick[8] = true;
        }
        this.loadList();
      });

      $('.sort thead .column').eq(9).on('click', () => {
        this.delDom();
        this.sortType = 'createDate';
        if (isClick[9]) {
          this.sortNum = 0;
          $('.sort thead .column').eq(9).append("<i aria-hidden='true' class='material-icons icon'>arrow_upward</i>");
          $('.sort thead .column').eq(9).addClass('sortable active asc desc');
          isClick[9] = false;
        } else {
          this.sortNum = 1;
          $('.sort thead .column').eq(9).append("<i aria-hidden='true' class='material-icons icon'>arrow_upward</i>");
          $('.sort thead .column').eq(9).addClass('sortable active asc');
          isClick[9] = true;
        }
        this.loadList();
      });
      $('.sort thead .column').eq(10).on('click', () => {
        this.delDom();
        this.sortType = 'processDateTime';
        if (isClick[10]) {
          this.sortNum = 0;
          $('.sort thead .column').eq(10).append("<i aria-hidden='true' class='material-icons icon'>arrow_upward</i>");
          $('.sort thead .column').eq(10).addClass('sortable active asc desc');
          isClick[10] = false;
        } else {
          this.sortNum = 1;
          $('.sort thead .column').eq(10).append("<i aria-hidden='true' class='material-icons icon'>arrow_upward</i>");
          $('.sort thead .column').eq(10).addClass('sortable active asc');
          isClick[10] = true;
        }
        this.loadList();
      });
    ...
    methods: {
      delDom() {
        ...
        for (let i = 0; i < sortDom.length; i++) {
          ...
          $('.sort thead .column').eq(i).find('i').empty();
          $('.sort thead .column').eq(i).removeClass('sortable active desc asc');
      ...
      loadList() {
        ...
        let data = {
          ...
          boutique: this.searchData.boutique.id,
          api: this.searchData.api.id,
          errorType: this.searchData.errorType.id,
      ...
      loadBoutique(vid, pid) {
        ...
        searchApiError(data).then(res => {
          ...
          this.vendorList.unshift({vendor_name: 'All'});
          this.apiList = res.data.apiList;
          this.apiList.unshift({name: 'All'});
          this.apiErrorTypeList = res.data.apiErrorTypeList;
          this.apiErrorTypeList.unshift({error_type_name: 'All'});
          setTimeout(() => {
            $('select').material_select();
            this.isLoading = false;
          }, 100);
      ...
      searchList() {
        this.searchArr = [];
        this.searchArr.push({id: 0, name: this.searchData.Date}, {id: 1, name: this.searchData.boutique.name}, {
          id: 2,
          name: this.searchData.api.name
        }, {id: 3, name: this.searchData.errorType.name}, {id: 4, name: this.searchData.brandId}, {
          id: 5,
...
<style lang="less" scoped>
  ...
  input[type=text]:focus:not([readonly]), input[type=text].invalid {
    border-bottom: 2px solid #871B55;
    box-shadow: none;
    & + label {

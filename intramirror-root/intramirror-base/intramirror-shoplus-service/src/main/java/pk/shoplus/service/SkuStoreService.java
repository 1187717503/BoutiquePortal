...
/**api------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
	...
	public List<Map<String,Object>> api_select_store(String product_code,String size,String vendor_id) throws Exception{
		String sql = "select ss.`sku_store_id` ,ss.`confirmed` ,ss.`store` ,ss.`reserved`,s.price,p.product_id,DATE_FORMAT(ss.`last_check`,'%Y-%m-%d %H:%i:%s') as last_check   from `product` p  \n"

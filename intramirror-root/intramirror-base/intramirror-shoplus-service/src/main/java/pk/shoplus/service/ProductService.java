...
public class ProductService {
    ...
    public List<Map<String, Object>> getProductByBrandID(String brandIds, Long vendorId) throws Exception {
        try {
            String sql = "select p.product_id from product p where p.enabled = 1 and p.vendor_id = '" + vendorId + "' and p.product_id in \n" +
                    "(\n" +
                    "select distinct pp.product_id from product_property pp where pp.enabled = 1 and pp.key_name = 'BrandID' and pp.`value` = \"" + StringUtils.escapeStr(brandIds) + "\" " +
                    ")";
     ...
     * @throws Exception
     */
    public Page getProductListByCondition(long pageNumber, long pageSize, String fieldNames, String tableName,
                                          String whereCondition, String orderBy, Object[] params) throws Exception {
        try {
            Page productPageList = productDao.getPageBySql(pageNumber, pageSize, fieldNames, tableName, whereCondition,
     ...
     * @throws Exception
     */
    public Page getProductPageList(long pageNumber, long pageSize, Map<String, Object> whereCondition, String orderBy)
            throws Exception {
        try {
            String fieldNames = "*";
            String tableName = "product";
            orderBy = Helper.isNullOrEmpty(orderBy) ? "product_id" : orderBy;
            Page productList = productDao.getPageBySql(pageNumber, pageSize, fieldNames, tableName, whereCondition,
                    orderBy, null, null);
            return productList;
        } catch (Exception e) {
            e.printStackTrace();
     ...
     * @throws Exception
     */
    public Page getProductPageWithVendorAndBrandAndCategory(long pageNumber, long pageSize,
                                                            Map<String, Object> whereCondition, String fieldNames, String orderBy, String whereStringInput)
            throws Exception {
    ...
    // 获取 whereString
    public String getProductWhereString(long pageNumber, long pageSize, Map<String, Object> whereCondition,
                                        String fieldNames, String orderBy, String whereStringInput) throws Exception {
        try {
            // 1. condition转换成 wherestr
            StringBuilder whereSql = new StringBuilder("");
    ...
    public List<Map<String, Object>> getProductInfoByCondition(String status, boolean enabled) throws Exception {
        try {
            ...
            StringBuilder sql = new StringBuilder("");
            sql.append(
                    "select p.*,v.vendor_name vendor_name,c.name category_name,b.chinese_name chinese_name,b.english_name english_name from product p \n")
     ...
     * @return
     */
    public List<Map<String, Object>> getProductInfoWithLimit(long pageNumber, long pageSize, String status,
                                                             boolean enabled, String vendor_id) throws Exception {
        try {
            StringBuilder sql = new StringBuilder("");
            sql.append(
                    "select p.*,v.vendor_name vendor_name,c.name category_name,b.chinese_name chinese_name,b.english_name english_name from product p ")
    ...
    public Page getProductList(Page page) throws Exception {
        try {
            ...
            StringBuilder condition = new StringBuilder(
                    "p.enabled=:p1 and s.enabled=:p2 and ss.enabled=:p3 and  p.`status`=:p4 ");
            // List<Object> paramList = new ArrayList<Object>();
            String orderBy = "";
            Object[] params = new Object[]{EnabledType.USED, EnabledType.USED, EnabledType.USED,
                    ProductStatusType.EXISTING};
            page = productDao.getPageBySql(page.pageNumber, page.pageSize, fieldNames, tableName, condition.toString(),
                    orderBy, params, "p.product_id", "p.product_id");
        } catch (Exception e) {
            e.printStackTrace();
            throw e;
        }
    ...
    public List<Map<String, Object>> getProductByIds(String shop_product_ids) throws Exception {
        try {
            StringBuilder sql = new StringBuilder("");
            sql.append(
                    "SELECT p.cover_img FROM shop_product sp LEFT JOIN product p ON sp.product_id = p.product_id WHERE sp.shop_product_id IN (");
    ...
    public List<Map<String, Object>> getProductSkuList(Long productId) throws Exception {
		 ...
		 */
        StringBuilder sb = new StringBuilder();
        sb.append("SELECT p.`product_code`,sk.`price`,sk.`sku_id` FROM product p ")
                .append(" INNER JOIN sku sk ON sk.`product_id`=p.`product_id` AND sk.`enabled`=1")
                .append(" WHERE p.`product_id`=:p1");
    ...
    public Page getProductCommentList(Long productId, long pageNumber, long pageSize, String orderBy) throws Exception {
        ...
        Page page = new Page();
        try {
            page = productDao.getPageBySql(pageNumber, pageSize, fieldNames, sb.toString(), whereCondition, orderBy,
                    params, null, null);
    ...
    public Page getProductList(Page page, String brand_id, List<Category> sub_category_ids, String min_price,
                               String max_price, String order_by, String product_name, String is_added, String vendor_id) throws Exception {
        try {
            ...
            StringBuilder condition = new StringBuilder(
                    "p.enabled=:p1 and s.enabled=:p2 and ss.enabled=:p3 and  p.`status`=:p4 and p.valid_at>:p5");
            ...
            page = productDao.getPageBySql(page.pageNumber, page.pageSize, fieldNames, tableName, condition.toString(),
                    orderBy, paramList.toArray(), "p.product_id", "p.product_id");
        } catch (Exception e) {
            e.printStackTrace();
            throw e;
        }
     ...
     * @throws Exception
     */
    public Page getAppProductPageWithVendorAndBrandAndCategory(long pageNumber, long pageSize,
                                                               Map<String, Object> whereCondition, String fieldNames, String orderBy, String whereStringInput)
            throws Exception {
        // SELECT a.*, b.vendor_name, c.chinese_name, c.english_name, d.name AS
        // category_name FROM product a LEFT JOIN vendor b on a.vendor_id =
        ...
        try {
            ...
            StringBuilder countSql = new StringBuilder("select count(*) from product a LEFT JOIN product_property e  ON a.product_id = e.product_id  LEFT JOIN sku f  ON  a.product_id = f.product_id  INNER JOIN product_property g ");
            ...
            sql.append(" and ")
                    .append("e.key_name = 'ColorCode'")
                    .append(" and ")
                    .append("g.key_name='BrandID'")
                    .append(" group by ")
                    .append("a.product_id");
...
//	}
     ...
     * @throws Exception
     */
    public Page getVendorProductPage(long pageNumber, long pageSize, Map<String, Object> whereCondition)
            throws Exception {
        ...
        try {
            ...
            String sql = getVendorProductSQL("t_pro.product_id,t_pro.product_code,t_pro.`name` as productName ,t_pp.brandID,t_pp.colorCode,t_pp.seasonCode,brand.english_name,brand.chinese_name,t_category.`name` as c1_name,t_category.c2_name,t_category.c3_name", whereCondition);
    ...
    public Map<String,Object> getBrand(String brandName) throws Exception {
        try {
            String sql  = "select b.brand_id,b.english_name from brand b where lower(b.english_name) = lower(\"" + brandName+ "\") and b.enabled = 1";
            logger.info("ProductService,getThreeCategory,sql:" + sql);
    ...
    public Map<String,Object> getBrandMapping(Long vendor_id,String brandName) throws Exception {
        try {
            String sql  = "select abm.brand_id,b.english_name from `api_brand_map`  abm\n"
                    ...
                    + "where ac.`vendor_id`  = "+vendor_id+" and lower(abm.`boutique_brand_name`)  =lower(\"" + brandName + "\")";
            logger.info("ProductService,getThreeCategory,sql:" + sql);

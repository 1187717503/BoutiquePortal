...
public class ApiMqService {
    ...
    public Map<String, Object> getApiMqWhereErrorType(String vendorId, String apiEndPointId) {
        List<Map<String, Object>> listMap = new ArrayList<Map<String, Object>>();
        try {
            String sql = " select * from api_mq am " +
                    " inner join api_configuration ac on(ac.enabled = 1 and ac.api_configuration_id = am.api_configuration_id) " +
                    " inner join api_end_point aep on(aep.enabled = 1 and aep.api_end_point_id = ac.api_end_point_id) " +
                    " where aep.api_end_point_id = '" + apiEndPointId + "' and ac.vendor_id = '" + vendorId + "' and am.enabled = 1";
            listMap = apiMqDao.executeBySql(sql, null);

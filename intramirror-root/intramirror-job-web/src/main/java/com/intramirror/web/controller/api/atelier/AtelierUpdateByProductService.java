    ...
    public Map<String, Object> updateProduct(String body, String storeID, String version, String type) {
        ...
        try {
            ...
            if (StringUtils.isBlank(storeID)) {
                return mapUtils.putData("ResponseStatus", "2000").putData("ErrorCode", "1").putData("ErrorMsg", "E001: Parameter StoreID is mandatory").putData(
                        "TimeStamp", new Date()).getMap();
            }

            if (StringUtils.isBlank(version) || !"1.0".equals(version)) {
                return mapUtils.putData("ResponseStatus", "2000").putData("ErrorCode", "1").putData("ErrorMsg", "E001: Parameter Version is mandatory").putData(
                        "TimeStamp", new Date()).getMap();
            }

            if (StringUtils.isBlank(body)) {
                return mapUtils.putData("ResponseStatus", "2000").putData("ErrorCode", "30").putData("ErrorMsg", "E030: Empty Request").putData("TimeStamp",
                        new Date()).getMap();
            }
            ...
            if (StringUtils.isBlank(boutique_id)) {
                return mapUtils.putData("ResponseStatus", "2000").putData("ErrorCode", "205").putData("ErrorMsg", "E205: Invalid boutique_id").putData(
                        "TimeStamp", new Date()).getMap();
            }
            ...
            logger.info("AtelierUpdateByProductServiceUpdateProduct,mapping,productOptions:" + JSONObject.toJSONString(productOptions) + ",bodyDataMap:"
                    + JSONObject.toJSONString(bodyDataMap) + ",eventName:" + eventName);
            ...
            logger.info("AtelierUpdateByProductServiceUpdateProduct,executeThreadTool,eventName:" + eventName + ",productOptions:" + JSONObject
                    .toJSONString(productOptions) + ",vendorOptions:" + JSONObject.toJSONString(vendorOptions) + ",fileUtils:" + JSONObject
                    .toJSONString(fileUtils) + ",storeCode:" + storeID);
            ...
            logger.info(
                    "Job_Run_Time,AtelierUpdateByProductService_UpdateProductThread,start:" + start_execute + ",end:" + end_execute + ",time:" + (end_execute
                            - start_execute));
            ...
            return mapUtils.putData("ResponseStatus", "2000").putData("ErrorCode", "1").putData("ErrorMsg",
                    "500: E500: failed:" + ExceptionUtils.getExceptionDetail(e)).putData("TimeStamp", new Date()).getMap();
        }

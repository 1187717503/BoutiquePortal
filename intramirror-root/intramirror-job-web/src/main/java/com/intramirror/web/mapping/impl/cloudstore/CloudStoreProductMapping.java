...
public class CloudStoreProductMapping implements IProductMapping {
    ...
    public ProductEDSManagement.ProductOptions mapping(Map<String, Object> bodyDataMap) {
        ...
        try {
            ...
            // handle product
            productOptions.setName(jsonObjectData.getString("title_en")).setCode(jsonObjectData.getString("mnf_code").trim()).setSeasonCode(
                    jsonObjectData.getString("season")).setBrandCode(brandID).setBrandName(jsonObjectData.getString("brand").trim()).setColorCode(colorCode)
                    .setColorDesc(jsonObjectData.getString("color_en")).setCategoryId(category_id).setCategory_name(jsonArray.toJSONString()).setDesc(
                    jsonObjectData.getString("desc_en")).setComposition(jsonObjectData.getString("material_en")).setCoverImg(jsonObjectData.getString("images"))
                    .setDescImg(jsonObjectData.getString("images")).setSalePrice(jsonObjectData.getString("stock_price")).setFullUpdateProductFlag(
                    full_update_product).setLast_check(new Date());
            ...
            if (vendor_id.equals("43") && StringUtils.isBlank(productOptions.getSeasonCode())) {
                productOptions.setSeasonCode("empty");
            }
            if (conn != null) {
                conn.close();
            }
            ...
            logger.info(
                    "cloidStoreUpdateByProductMapping,errorMessage:" + ExceptionUtils.getExceptionDetail(e) + ",bodyDataMap:" + new Gson().toJson(bodyDataMap));
            if (conn != null) {
                conn.close();
            }
            e.printStackTrace();
